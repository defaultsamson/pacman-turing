...
% - Make AI have a random offset so it doesn't hit ball straight every single time
...
body proc updateAI
    ...
    var target := floor (-(ballChangeY (firstBallID) / ballChangeX(firstBallID) * (ballx (firstBallID) + paddlex(1))) + bally (firstBallID))
   
    % If the targeted ball is going away or is beyond the halfway line then target the middle
    if ballChangeX (firstBallID) > 0 or ballx (firstBallID) > halfx then
	target := halfy
    end if
